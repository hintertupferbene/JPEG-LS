...
void check_range(int param, const char *name, int low, int high)
{
...
int write_jpegls_extmarker_encoder(FILE *out, jpeg_ls_header *jp, int IDtype, char *mapfilename)
{
  int ct=0;
  ...
  if (IDtype == LSE_MAPTABLE)
    ...
    /* Create the table */
    jp->TABLE[TID] =
      (unsigned int *) safecalloc ((MAXTAB+1)*sizeof(unsigned int), 1);
    ...
    for (i=0; i <= MAXTAB; i++)
      ...
      jp->TABLE[TID][i] = read_n_bytes(tablefile, (int)Wt);
      if (feof(tablefile))
        {
    ...
    ct += write_n_bytes(out, (int)length, 2);    /* marker length */
    ct += write_n_bytes(out, (int)LSE_MAPTABLE, 1);  /* LSE marker id */
    ct += write_n_bytes(out, (int)TID, 1);      /* table id */
    ct += write_n_bytes(out, (int)Wt, 1);      /* width of table entries in bytes */

    for (i=0; i<=MAXTAB; i++)
      ct += write_n_bytes(out, (int)jp->TABLE[TID][i], (int)Wt);
...
/* Write out LSE header to JLS file - only supports type 1 and 2 currently */
int write_jpegls_extmarker_decoder(FILE *out, jpeg_ls_header *jp)
{
    int ct=0;
