  ...
  else
  {
    if (bpp16==FALSE)
    {
      ...
      else if (head_scan[0]->Wt == 2)
      {
        ...
        {
          indx = *(line+i);
          *(line16+i) = (unsigned short) maptable[indx];
        }
      ...
      else if (head_scan[0]->Wt == 3)
      {
        ...
        {
          indx = *(line+i);
          *(line8_3 + (i*3)) =  (unsigned char) (maptable[indx] >> 16);
          *(line8_3 + (i*3) + 1) = (unsigned char) (maptable[indx] >> 8);
          *(line8_3 + (i*3) + 2) = (unsigned char) (maptable[indx]);
        }
...
int initialize(int argc, char *argv[])
{
  ...
  set_thresholds(head_scan[0]->alp, head_scan[0]->NEAR,
    &(head_scan[0]->T1), &(head_scan[0]->T2), &(head_scan[0]->T3));
  T1=head_scan[0]->T1;
  T2=head_scan[0]->T2;
  T3=head_scan[0]->T3;
  ...
  if (head_scan[0]->RES != DEFAULT_RESET) {
    fprintf(stderr,"ERROR: Version compiled for fixed RESET=%d parameter: "
  ...
  for (i=0;i<components;i++) {
    ...
    if ((!MULTI) && ((samplingx[i]!=1)
        || (samplingy[i]!=1)) && (components>1))
    {
    fprintf(stderr, "\nERROR: Cannot use -P (PPM output) "
      "with subsampled compressed components\n");
      exit(10);
    }
  }
  ...
  if ((MULTI) && (out_files) && (out_files!=components)) {
    fprintf(stderr,"ERROR: Number of files, %d, for output must be equal to "
  ...
  if (lossy==TRUE)
    ...
    fprintf(msgfile,
      "Near-lossless mode: NEAR = %d  beta = %d  qbeta = %d\n",NEAR,BETA,qbeta);
    }
  ...
  if ( verbose ) {
    if (!MULTI)
      fprintf(msgfile,
        "Image: cols=%d rows=%d alpha=%d comp=%d mode=%d (%s)\nParameters: T1=%d T2=%d T3=%d RESET=%d limit=%d",
