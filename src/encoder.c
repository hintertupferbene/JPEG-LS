...
inline void read_one_line(pixel* line, int cols, FILE* infile)
{
  ...
  if (bpp16==FALSE)
  {
    ...
    if (fread(line8, 1, cols, infile) != (unsigned int)cols)
  ...
  {
    if (fread(line, 2, cols, infile) != (unsigned int)cols)
      fprintf(stderr,"Input file is truncated");
...
void initialize(int argc, char *argv[]) 
{
  ...
  const char   *outfilename = OUTFILE COMPSUFFIX;
...
int main (int argc, char *argv[])
{
  ...
  for (n_s=0;n_s<number_of_scans;n_s++) {
    ...
    if (n_s==0) {
      ...
      if ( need_lse != 0 )
        all_header += write_jpegls_extmarker_encoder(OUT, head_scan[n_s], LSE_PARAMS, mappingtablefilename);
      if ( need_table != 0 )
        all_header += write_jpegls_extmarker_encoder(OUT, head_scan[n_s], LSE_MAPTABLE, mappingtablefilename);
...
/***********************************************************************/
  ...
  if ( verbose )
      fprintf(msgfile,"Marker segment bytes: %d\n",all_header);

  fprintf(msgfile,"Total bits out: %ld  Symbols in: %ld  %5.3f bps  (%5.3f : 1)\n",
           tot_out,
